...
class TestCredentialManager:
    ...
    def test_credential_encryption(self, credential_manager, sample_credentials):
        ...
        cred_file = storage_path / f"{service}.cred"
        if cred_file.exists():
    ...
    def test_audit_logging(self, credential_manager, sample_credentials):
        ...
        # Check audit log file was created and contains the operation
        audit_log_path = credential_manager.audit_log
        assert audit_log_path.exists(), "Audit log file should be created"
        
        # Read audit log content
        with open(audit_log_path, 'r') as f:
            log_content = f.read()
        
        # Should contain the service name and action
        assert service in log_content
        assert "store_credentials" in log_content
        assert "true" in log_content.lower()  # success should be true
