#!/usr/bin/env python3
"""
Thread Pool Manager for Universal Document Converter
...
class ThreadPoolManager:
    ...
    def __init__(self):
        ...
        self.logger.info(f"ThreadPoolManager initialized: CPUs={self.cpu_count}, "
                        f"Memory={self.memory_gb:.1f}GB, Max threads={self.max_system_threads}")
    ...
    def _calculate_max_threads(self) -> int:
        ...
        # Memory-based limit: 1 thread per 100MB of RAM (conservative)
        memory_based_limit = int(self.memory_gb * 10)
        ...
        max_threads = min(cpu_based_limit, memory_based_limit, system_limit)
    ...
    def _monitor_resources(self):
        ...
        while not self._stop_monitor.is_set():
            try:
                ...
                # Apply backpressure if resources are constrained
                if cpu_percent > 90 or memory_percent > 85:
                    ...
                    if not self._backpressure_event.is_set():
                        self.logger.info("Resource usage normalized, removing backpressure")
                        self._backpressure_event.set()
    ...
    def shutdown_all(self, wait: bool = True, timeout: float = 30):
        ...
        with self._pools_lock:
            ...
            for pool_name in pool_names:
                self.shutdown_pool(pool_name, wait=wait, timeout=timeout/len(pool_names))
